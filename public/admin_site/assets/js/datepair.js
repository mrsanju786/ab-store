/*!
 * datepair.js v0.4.19 - A javascript plugin for intelligently selecting date and time ranges inspired by Google Calendar.
...
(function () {
  ...
  Datepair.prototype = {
    ...
    _bindChangeHandler: function () {
      ...
      if (jq) {
        jq(this.container).on(
          "change.datepair",
          jq.proxy(this.handleEvent, this)
        );
      } else {
    ...
    _dateChanged: function (target) {
      ...
      if (!startDate || !endDate) {
        if (this.settings.defaultDateDelta !== null) {
          if (startDate) {
            var newEnd = new Date(
              startDate.getTime() + this.settings.defaultDateDelta * _ONE_DAY
            );
            this.settings.updateDate(this.endDateInput, newEnd);
          } else if (endDate) {
            var newStart = new Date(
              endDate.getTime() - this.settings.defaultDateDelta * _ONE_DAY
            );
      ...
      if (
        this.settings.anchor == "start" &&
        hasClass(target, this.settings.startClass)
      ) {
        var newDate = new Date(startDate.getTime() + this.dateDelta);
        this.settings.updateDate(this.endDateInput, newDate);
      } else if (
        this.settings.anchor == "end" &&
        hasClass(target, this.settings.endClass)
      ) {
        var newDate = new Date(endDate.getTime() - this.dateDelta);
        this.settings.updateDate(this.startDateInput, newDate);
      } else {
        if (endDate < startDate) {
          var otherInput = hasClass(target, this.settings.startClass)
            ? this.endDateInput
            : this.startDateInput;
    ...
    _timeChanged: function (target) {
        ...
        if (this.settings.defaultTimeDelta !== null) {
          ...
          if (startTime) {
            endTime = this._setTimeAndReturn(
              this.endTimeInput,
              new Date(startTime.getTime() + this.settings.defaultTimeDelta)
            );
            this._doMidnightRollover(startTime, endTime);
          } else if (endTime) {
            startTime = this._setTimeAndReturn(
              this.startTimeInput,
              new Date(endTime.getTime() - this.settings.defaultTimeDelta)
            );
            this._doMidnightRollover(startTime, endTime);
          }
        } else {
      ...
      if (
        ...
        if (+startDate == +endDate && endTime < startTime) {
          var thisInput = hasClass(target, this.settings.endClass)
            ? this.endTimeInput
            : this.startTimeInput;
          var otherInput = hasClass(target, this.settings.startClass)
            ? this.endTimeInput
    ...
    _doMidnightRollover: function (startTime, endTime) {
      ...
      if (
        ...
        ((newDelta >= 0 && this.timeDelta < 0) ||
          (newDelta < 0 && this.timeDelta >= 0))
      ) {
        if (this.settings.anchor == "start") {
          this.settings.updateDate(
            this.endDateInput,
            new Date(endDate.getTime() + offset)
          );
          this._dateChanged(this.endDateInput);
        } else if (this.settings.anchor == "end") {
          this.settings.updateDate(
            this.startDateInput,
            new Date(startDate.getTime() - offset)
          );
