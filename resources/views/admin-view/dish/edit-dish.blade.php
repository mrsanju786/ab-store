                            ...
                            <div class="d-flex flex-column scroll-y me-n7 pe-7" id="kt_modal_add_role_scroll">
                                ...
                                <div class="fv-row mb-10 fv-plugins-icon-container">
                                    ...
                                    <select class="form-select form-select-solid" aria-label="Select example" name="discount_id" id="discount_id" data-control="select2">
                                        ...
                                        <option value="{{$discounts->id}}" {{($discounts->id == $dish->discount_ids)?'selected':''}}>{{$discounts->discount_name}}</option>
                                        @endforeach
...
$(document).ready(function() {
    ...
    if(cat_id){
        $.ajax({
            ...
            url: "{{ url('admin/get-branch-tax') }}/" + cat_id,
            type: "GET",
            success: function(response) {
                var Dish_tax = "{{$dishTax}}";
                $.each(response,function(key, value)
                {
